...
```
v3 Q-Learning  → 用 Q 表格，直接学 Q(s,a)，不需要知道 P
v4 SARSA       → 也用 Q 表格
v5 DQN         → 用神经网络近似 Q(s,a)

v8 Actor-Critic → Actor 学策略 π(a|s)，Critic 学 V(s)
v9 A2C         → Advantage = Q(s,a) - V(s)，同时用到了两者！
```
...
- **Q-Learning**：用 Q 表格记录每个 $(s, a)$ 的价值，通过 Bellman 方程迭代更新
- **SARSA**：与 Q-Learning 类似，但使用 On-Policy 更新

这些方法将真正"求解" MDP，找到从起点到终点的最优路径。
